...
class euvps_sem_seg_head(VITA):
    ...
    def __init__(self, cfg, in_channel):
        ...
        self.reduce_block = [DilatedParallelConvBlockD2(in_dim, num_queries // 4).cuda() for in_dim in self.query_in_channels]
    ...
    def forward(self, features):
        ...
        features = [x.view(x.shape[0], x.shape[1], -1) for x in stage_features]

        _shape_embed = []
        for feat, s_embed in zip(features, self.region_embed):
            _shape_embed.append(s_embed(feat))

        _shape_embed_reduce = []
        for feat, reduce in zip(_shape_embed, self.reduce_block):
            _shape_embed_reduce.append(reduce(feat.unsqueeze(2)))

        shape_query = torch.cat(_shape_embed_reduce, dim=1).squeeze(2)
        ...
        for i in range(l // clip_len):
            outputs, output_q = self.transformer_enc_dec(shape_query[i*clip_len:(i+1)*clip_len].unsqueeze(0), pre_memory, output_q, clip_len)
            out = torch.einsum("lbqc, lbqhw -> lbhw", outputs["pred_mask_embed"], mask_proposals[:,i*clip_len:(i+1)*clip_len, :, :, :])
            output.append(out)
