...
w64_stage1 = [64]
w64_stage2 = [64, 128]
w64_stage3 = [64, 128, 256]
w64_stage4 = [64, 128, 256, 512]

w48_stage1 = [64]
w48_stage2 = [48, 96]
w48_stage3 = [48, 96, 192]
w48_stage4 = [48, 96, 192, 384]
...
def build_hrnet(config, input_shape, **kwargs):
    cfg = None
    if config.MODEL.BACKBONE.MODEL == "w30":
        cfg = create_w30_config()
    elif config.MODEL.BACKBONE.MODEL == "w48":
        cfg = create_w48_config()

    model = HighResolutionNet(cfg, **kwargs)

    if config.MODEL.BACKBONE_FILE:
        model_state = torch.load(config.MODEL.BACKBONE_FILE)
        model.load_state_dict(model_state)

    return model


def create_w30_config():
    cfg = CN()
    cfg.MODEL = CN()
    ...
    cfg.MODEL.EXTRA.STAGE1.NUM_RANCHES = 1
    ...
    cfg.MODEL.EXTRA.STAGE2.NUM_CHANNELS = [30, 60]
    ...
    cfg.MODEL.EXTRA.STAGE3.NUM_CHANNELS = [30, 60, 120]
    ...
    cfg.MODEL.EXTRA.STAGE4.NUM_CHANNELS = [30, 60, 120, 240]
    cfg.MODEL.EXTRA.STAGE4.FUSE_METHOD = "SUM"

    return cfg


def create_w48_config():
    cfg = CN()
    cfg.MODEL = CN()
    cfg.MODEL.NAME = 'cls_hrnet'
    cfg.MODEL.INIT_WEIGHTS = True
...
if __name__ == '__main__':
    ...
    x = torch.randn(1, 3, 256, 448).cuda()
    y = model(x)
