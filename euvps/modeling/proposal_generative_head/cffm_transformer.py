...
class WindowAttention3d3(nn.Module):
    ...
    def __init__(self, dim, expand_size, window_size, focal_window, focal_level, num_heads,
                 qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., pool_method="none", focal_l_clips=[7, 1, 2],
    ...
    def forward(self, x_all, mask_all=None, batch_size=None, num_clips=None):
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level - 1):
                ...
                (k_pooled_k, v_pooled_k) = map(
                    lambda t: self.unfolds[k](t).view(
                        B0, C, self.unfolds[k].kernel_size[0], self.unfolds[k].kernel_size[1], -1).permute(0, 4, 2, 3,
                                                                                                           1).contiguous(). \
            ...
            for k in range(len(self.focal_l_clips)):
                ...
                unfolded_mask = self.unfolds_clips[k](mask.unsqueeze(0).unsqueeze(1)).view(
                    1, 1, self.unfolds_clips[k].kernel_size[0], self.unfolds_clips[k].kernel_size[1], -1).permute(0, 4,
                                                                                                                  2, 3,
                                                                                                                  1).contiguous(). \
                    view(nWh * nWw // stride // stride, -1, 1)
                ...
                if (not focal_l_big_flag):
                    (k_pooled_k, v_pooled_k) = map(
                        lambda t: self.unfolds_clips[k](t).view(
                            B0, C, self.unfolds_clips[k].kernel_size[0], self.unfolds_clips[k].kernel_size[1],
                            -1).permute(0, 4, 2, 3, 1).contiguous(). \
        ...
        attn = (q_windows @ k_all.transpose(-2,
                                            -1))  # B0*nW, nHead, window_size*window_size, focal_window_size*focal_window_size
        ...
        if self.pool_method != "none" and self.focal_level > 1:
            ...
            for k in range(self.focal_level - 1):
                ...
                if mask_all[0][k + 1] is not None:
                    attn[:, :, :window_area, offset:(offset + (self.focal_window + 2 ** k - 1) ** 2)] = \
                        attn[:, :, :window_area, offset:(offset + (self.focal_window + 2 ** k - 1) ** 2)] + \
                        mask_all[0][k + 1][:, :, None, None, :].repeat(attn.shape[0] // mask_all[0][k + 1].shape[1], 1,
                                                                       1, 1, 1).view(-1, 1, 1,
                                                                                     mask_all[0][k + 1].shape[-1])
            ...
            for k in range(len(self.focal_l_clips)):
                ...
                relative_position_bias_to_windows = self.relative_position_bias_table_to_windows_clips[k][:,
                                                    relative_position_index_k.view(-1)].view(
                    -1, self.window_size[0] * self.window_size[1], (kernel_size_true) ** 2,
                )
                attn[:, :, :window_area, offset:(offset + (kernel_size_true) ** 2)] = \
                    attn[:, :, :window_area,
                    offset:(offset + (kernel_size_true) ** 2)] + relative_position_bias_to_windows.unsqueeze(0)
...
class CffmTransformerBlock3d3(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, shift_size=0,
                 ...
                 focal_level=1, focal_window=1, use_layerscale=False, layerscale_value=1e-4, focal_l_clips=[7, 2, 4],
                 focal_kernel_clips=[7, 5, 3]):
        super().__init__()
        self.dim = dim
        ...
        self.attn = WindowAttention3d3(
            ...
            focal_l_clips=focal_l_clips, focal_kernel_clips=focal_kernel_clips)
    ...
    def forward(self, x):
        ...
        if self.focal_level > 1 and self.pool_method != "none":
            ...
            for k in range(self.focal_level - 1):
                ...
                x_windows_noreshape = window_partition_noreshape(x_level_k.contiguous(),
                                                                 window_size_glo)  # B0, nw, nw, window_size, window_size, C
                nWh, nWw = x_windows_noreshape.shape[1:3]
                if self.pool_method == "mean":
                    x_windows_pooled = x_windows_noreshape.mean([3, 4])  # B0, nWh, nWw, C
                elif self.pool_method == "max":
                    x_windows_pooled = x_windows_noreshape.max(-2)[0].max(-2)[0].view(B0, nWh, nWw,
                                                                                      C)  # B0, nWh, nWw, C
                elif self.pool_method == "fc":
                    x_windows_noreshape = x_windows_noreshape.view(B0, nWh, nWw, window_size_glo * window_size_glo,
                                                                   C).transpose(3, 4)  # B0, nWh, nWw, C, wsize**2
                    ...
                    x_windows_noreshape = x_windows_noreshape.view(-1, window_size_glo, window_size_glo, C).permute(0,
                                                                                                                    3,
                                                                                                                    1,
                                                                                                                    2).contiguous()  # B0 * nw * nw, C, wsize, wsize
                    x_windows_pooled = self.pool_layers[k](x_windows_noreshape).view(B0, nWh, nWw,
                                                                                     C)  # B0, nWh, nWw, C
            ...
            for k in range(len(self.focal_l_clips)):
                ...
                if H != H_pool or W != W_pool:
                    x_level_k = F.interpolate(x_level_k.permute(0, 3, 1, 2), size=(H_pool, W_pool),
                ...
                if self.pool_method == "mean":
                    ...
                    x_windows_noreshape = x_windows_noreshape.view(B0, nWh, nWw, window_size_glo * window_size_glo,
                                                                   C).transpose(3, 4)  # B0, nWh, nWw, C, wsize**2
                    x_windows_pooled = self.pool_layers_clips[k](x_windows_noreshape).flatten(
                        -2)  # B0, nWh, nWw, C
                elif self.pool_method == "conv":
                    x_windows_noreshape = x_windows_noreshape.view(-1, window_size_glo, window_size_glo, C).permute(0,
                                                                                                                    3,
                                                                                                                    1,
                                                                                                                    2).contiguous()  # B0 * nw * nw, C, wsize, wsize
                    x_windows_pooled = self.pool_layers_clips[k](x_windows_noreshape).view(B0, nWh, nWw,
                                                                                           C)  # B0, nWh, nWw, C
        ...
        attn_windows = self.attn(x_windows_all_clips, mask_all=x_window_masks_all_clips, batch_size=B0,
                                 num_clips=D0)  # nW*B0, window_size*window_size, C
...
class BasicLayer3d3(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, expand_size, expand_layer="all",
                 ...
                 focal_l_clips=[16, 8, 2], focal_kernel_clips=[7, 5, 3]):
