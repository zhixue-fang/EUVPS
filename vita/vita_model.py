...
class Vita(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        # print("backbone_output_shape:", backbone.output_shape())
    ...
    def inference(self, batched_inputs):
        ...
        num_frames = batched_inputs.shape[0]
        to_store = self.device if num_frames <= 36 else "cpu"

        for i in range(math.ceil(num_frames / self.test_run_chunk_size)):
            images = batched_inputs[i*self.test_run_chunk_size : (i+1)*self.test_run_chunk_size]
        ...
        # interim_size = images.shape[-2:]
        # image_size = images.shape[-2:]  # image size without padding after data augmentation
        #
        ...
        interim_size = batched_inputs.shape[-2:]
        image_size = batched_inputs.shape[-2:]  # image size without padding after data augmentation

        out_height = batched_inputs.shape[-2]  # raw image size before data augmentation
        out_width = batched_inputs.shape[-1]
